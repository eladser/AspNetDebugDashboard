...
public class PerformanceTests
{
    ...
    public async Task DebugRequestMiddleware_ScalabilityTest_ShouldMaintainPerformance(int requestCount)
    {
        ...
        // Assert - Performance should scale linearly (adjusted for realistic test environment performance)
        var averageTimePerRequest = (double)stopwatch.ElapsedMilliseconds / requestCount;
        averageTimePerRequest.Should().BeLessThan(50); // Less than 50ms per request on average (adjusted from 10ms)
    }
